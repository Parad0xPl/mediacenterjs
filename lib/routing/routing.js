...
var mapRoutes = function (verbose, routeKey, routeValue, prefix, appName, app, routesFilePath) {
    ...
    if (routesMap.hasOwnProperty(routeKey)) {
        ...
        verbose && logger.info('     %s %s -> %s', method.toUpperCase(), routesPath, routeKey);// jshint ignore:line
    } else {
        logger.errror('\n   Broken route reference found.');
    }
};
var addRoute = function(appName, isPlugin) {// jshint ignore:line
    var verbose = options.verbose;
    var appDirectory = isPlugin ? 'node_modules' : 'apps';
    var appsPath = path.join(__dirname, '../..', appDirectory, appName);

    verbose && logger.info('\n   %s:', appName);// jshint ignore:line
    ...
    for (var key in obj) {
        ...
        // "reserved" exports
        if (~['name', 'prefix', 'engine', 'before'].indexOf(key)) {// jshint ignore:line
          continue;
        }
