...
/* Constants */
const CONFIG_PATH = './configuration/config.ini';
const CONFIG_TEMPLATE_PATH = './configuration/config-template.ini';
...
exports.initializeConfiguration = function () {
    if (!fs.existsSync(CONFIG_PATH)) {
        ...
        if(fs.existsSync(CONFIG_TEMPLATE_PATH)) {
          template = fs.readFileSync(CONFIG_TEMPLATE_PATH, 'utf-8');
        }else{
          console.error("Template config missing".red);
          process.exit(1);
        }
...
exports.saveSettings = function(settings, callback) {
    ...
    fs.writeFile(CONFIG_PATH, ini.stringify(config), function(err){
        if(err) {
            callback(err);
        } else {
            callback(null);
