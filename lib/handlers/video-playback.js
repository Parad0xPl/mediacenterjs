...
exports.startPlayback = function(response, platform, mediaObject, type) {
    if (!mediaObject.filePath) {
      logger.error('File '+ url + ' not found, did you move or delete it?', err); // jshint ignore:line
    ...
    checkProgression(mediaObject.id, type, function(data) {
        ...
        // if (platform === 'desktop') {
          getDuration(response, fileUrl, fileName, mediaObject, type, function (duration) {
              var webFriendlyOutputPath = fileUrl.substr(config.tvpath.length);
              var fileInfo = {
                  'outputPath'    : "/tv/data"+webFriendlyOutputPath,
                  'fileName'      : fileName,
                  'duration'      : duration,
...
var getDuration = function (response, url, file, mediaObject, type, callback) { // jshint ignore:line
...
var checkProgression = function(mediaID, type, callback) { // jshint ignore:line
    var findData = {};
    ...
    ProgressionMarker.find({ where: findData }).then(function (marker) { // jshint ignore:line
        if (!marker) {
            callback(empty);
            return;
        }
