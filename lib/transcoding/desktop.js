...
exports.transcode = function(mediaID, type, response, url, file, outputPath, ExecConfig, CurrentTranscodings) { // jshint ignore:line
    ...
    if (CurrentTranscodings.isTranscoding(url)) {
        ...
        var child = exec(ffmpeg, ExecConfig, function (err) {
            if (err) {
                ...
                ProgressionMarker.find({ where: progData }).then(function (marker) {
